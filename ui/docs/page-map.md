...
| `#/web/settings/users/:id` | 用户详情 | 身份、状态、会话和账号操作 |
...
- 历史 SSH 字段仅用于服务端 legacy 数据保留，客户端不提供新建、绑定、扫描或日常管理入口。
...
- 脚本契约失败、Agent 离线、操作失败、日志工具失败、执行中断、日志断连和超长日志。
