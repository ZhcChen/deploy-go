...
- 节点接入顺序固定为绑定凭证、扫描 host key、人工确认指纹、检查连接；扫描不得自动信任。
- App 只展示节点凭证绑定状态与健康结果，不生成、删除或重新绑定凭证。
...
| 节点 onboarding | 凭证与 host key 状态 loading 时禁用后续步骤；未绑定提示先选择凭证 | 能力信息失败不清除已确认指纹；主体失败显示整页重试 | 绑定、扫描、确认、检查分别维护 pending / success | 任一步失败停留当前阶段并允许重试，不自动信任 | 普通用户只读查看，修改请求返回权限不足 |
