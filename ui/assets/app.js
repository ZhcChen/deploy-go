(function bootstrapDeployGoUI() {
  ...
  function renderEntry() {
    return `<div class="entry">
      <header class="entry__bar"><div class="brand"><span class="brand__mark"><img src="./assets/logo.svg" alt=""></span><span>Deploy Go</span></div><span class="muted">UI Design Source</span></header>
  ...
  function webShell(content, active, title, subtitle, actions = "") {
    ...
    return `<div class="web-shell">
      <aside class="sidebar"><a class="brand" href="#/entry"><span class="brand__mark"><img src="./assets/logo.svg" alt=""></span><span>Deploy Go</span></a>
  ...
  function renderLogin(mobile=false) {
    const form=`<div class="login-panel"><div class="login-brand"><span class="brand__mark"><img src="./assets/logo.svg" alt=""></span><div><strong>Deploy Go</strong><span>部署控制服务</span></div></div><div><h1>登录</h1><p>使用管理员分配的账号进入控制台。</p></div>${state.scenario==="session-expired"?`<div class="notice">${icon("alert")} 会话已失效，请重新登录。</div>`:""}${state.loginError?`<div class="notice notice--danger">${icon("alert")} 邮箱或密码不正确，请重新输入。</div>`:""}<form data-login-form><div class="field"><label for="login-email">登录邮箱</label><input id="login-email" name="email" type="email" required value="${mobile?"lin@deploy.go":"chen@deploy.go"}" autocomplete="username"></div><div class="field"><label for="login-password">密码</label><input id="login-password" name="password" type="password" required value="deploygo123" autocomplete="current-password"></div><button class="btn btn--primary" type="submit">登录</button></form><p class="login-help">账号由唯一管理员创建和分配。</p></div>`;
  ...
  function renderMobileMineDetail(page) {
    ...
    const pages = {
      ...
      about: ["关于 Deploy Go", `<section class="about-hero"><span class="brand__mark"><img src="./assets/logo.svg" alt=""></span><h2>Deploy Go</h2><p>轻量级自动化部署服务</p></section><section class="mobile-section"><div class="key-list"><div class="key-row"><span>脚本契约</span><strong>Schema v1</strong></div><div class="key-row"><span>开源许可</span><strong>待确定</strong></div><div class="key-row"><span>服务状态</span><strong class="text-success">运行正常</strong></div></div></section>`],
    };
