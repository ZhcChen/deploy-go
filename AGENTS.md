...
- **修改范围硬性边界**：本项目排查、修复和优化部署问题时，只允许修改 `deploy-go` 本项目自身的代码、测试、文档、协议、Agent、控制面和部署工具。严禁修改被部署的任何业务应用代码、业务应用配置、业务应用脚本、业务应用仓库内容或业务应用发布物；业务应用的问题必须通过本项目的控制面、Agent、协议、诊断和部署逻辑解决。
