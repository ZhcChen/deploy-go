...
assert_contains "$DEPLOY_SCRIPT" 'DEPLOY_BUILD_MODE="${DEPLOY_BUILD_MODE:-remote}"'
assert_contains "$DEPLOY_SCRIPT" 'DEPLOY_BUILD_HOST="${DEPLOY_BUILD_HOST:-$DEPLOY_HOST}"'
assert_contains "$DEPLOY_SCRIPT" 'DEPLOY_BUILD_PROXY_URL="${DEPLOY_BUILD_PROXY_URL:-}"'
assert_contains "$DEPLOY_SCRIPT" 'DEPLOY_BUILD_REGISTRY_MIRROR="${DEPLOY_BUILD_REGISTRY_MIRROR:-}"'
assert_contains "$DEPLOY_SCRIPT" 'git archive --format=tar HEAD'
assert_contains "$DEPLOY_SCRIPT" '/var/lib/deploy-go-builder/build.'
assert_contains "$DEPLOY_SCRIPT" 'deploy/production/remote-build.sh'
assert_contains "$DEPLOY_SCRIPT" '--proxy-url "$DEPLOY_BUILD_PROXY_URL"'
assert_contains "$DEPLOY_SCRIPT" '--registry-mirror "$DEPLOY_BUILD_REGISTRY_MIRROR"'
assert_contains "$DEPLOY_SCRIPT" 'REMOTE_BUILD'
...
assert_contains "$REPO_ROOT/deploy/production/remote-build.sh" 'docker buildx version'
...
assert_contains "$REPO_ROOT/deploy/production/remote-build.sh" 'node:22-alpine'
assert_contains "$REPO_ROOT/deploy/production/remote-build.sh" '/var/lib/deploy-go-builder/'
