...
先创建并推送与 Cargo 版本一致的 `v0.3.1` tag。`Build Release Artifacts` 只会从该 tag 指向的提交构建并发布 API/Web Release 产物；Agent 在正式部署流程中仍由部署机本机构建：

```bash
DEPLOY_SOURCE=release \
DEPLOY_RELEASE_TAG=v0.3.1 \
bash deploy/production/deploy.sh
```
...
Agent 0.3.1 在 `agent/src/artifact_transfer.rs` 中使用
