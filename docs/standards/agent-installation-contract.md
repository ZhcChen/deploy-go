...
- `agent_version` 与 `executor_version` 必须相同，并与 API 当前发布版本一致；Agent 控制协议 v8 与 executor 本机 release 协议 v2 必须成对兼容。
...
- `make privileged-release-check`：协议 v8、executor v2、签名授权、bundle、环境白名单、生命周期、API/Web 和旧 release 兼容聚合检查。
