...
控制面应先升级到支持 v15，再逐节点安装 v15 配对发布物。v11-v14 Agent 在兼容期继续 heartbeat、部署、PTY 和 Env 任务，但节点详情显示不支持遥测；v15 Agent 连接仅支持更低协议的旧控制面时协商降级，不发送 telemetry。telemetry 属于 Agent 控制协议能力，不是 executor capability，也不得成为部署门禁。
