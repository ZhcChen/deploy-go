...
- `system_inspect`
- `deployment_execute`（历史任务类型，不向 v11 Agent 下发）
- `health_diagnose`
- `git_refs_query`
- `deployment_prepare`
- `deployment_release`
...
取消使用独立的 `task_cancel` 控制消息。在线自升级、文件管理、任意 shell 和任意 Make target 不属于普通任务能力。PTY 输入是已授权终端会话的短生命周期字节流，不是 `task_dispatch` payload，也不得写入 durable task journal、任务日志或结果。
...
PTY 消息和方向固定为：主控到 Agent 的 `terminal_open`、`terminal_input`、`terminal_resize`、`terminal_close`，以及 Agent 到主控的 `terminal_opened`、`terminal_output`、`terminal_exited`。每个发送方向分别维护严格递增且不可重复的 `sequence`，避免并发输入与输出争用同一序号；主控方向以 `terminal_open.sequence=0` 开始，Agent 方向以 `terminal_opened.sequence=1` 开始。收到错误会话、该方向重复或非预期序号必须拒绝并关闭会话，输入不进入 durable journal，也不得重放。
...
`terminal_close.reason` 使用严格枚举，覆盖管理员关闭、浏览器断开、授权撤销、空闲超时、最长存活超时和协议错误；`terminal_exited` 返回严格退出原因和可空退出码，覆盖进程退出、对端断开、输出超限及 executor 不可用。close 必须幂等。任一必要链路断开、身份或 Agent 能力失效、超时、shell 退出时，系统必须最终清理进程组。首版不跨 API 或 Agent 重启恢复会话，也不静默创建替代 root shell。
...
v11 `env_sync` payload 包含 `env_sync_id`、应用 Slug、`.env` 文件名、Env version、SHA-256 digest、`write`/`delete` 动作和 opaque `secret_lease_id`，不包含 Env 明文或主控指定的绝对路径。
...
v15 `runtime_probe` 任务用于平台应用列表的异步真实运行状态探测。payload 只
包含 `runtime_status_id`、`probe_type`（`http` / `tcp`）、`port`、`path`、
`expected_status` 和 `timeout_ms`；目标地址固定为节点本机 `127.0.0.1`，
不接受 host、URL、命令或环境变量。`runtime_probe_v1` 是独立 capability，
只发给协议 v15 且声明该 capability 的 Agent；v14 及更早 Agent 不会被下发
该任务，也不影响其部署、PTY、Env 同步或 workspace 任务。

- HTTP 探测对 `http://127.0.0.1:<port><path>` 发起 GET，按
  `expected_status` 与超时收敛结果。
- TCP 探测只连接 `127.0.0.1:<port>`，不发送业务数据。
- `task_result.data` 只允许 `checked_at`、`port`、`http_status` 等非敏感
  结构化字段，不得包含响应正文、Header、命令输出或节点路径。

未升级 Agent 或能力缺失由主控收敛为 `runtime_probe_protocol_unsupported` /
