...
两阶段部署的 `release-version` 由主控在生成部署预览时自动创建，确认部署复用预览中的版本并由 snapshot 完整性校验保护，管理端不得要求用户填写。`modules` 仍以逗号分隔字符串传给业务脚本；部署目标必须在该字段的 JSON Schema 中使用 `x-options` 声明 1 到 32 个可选模块。管理端允许多选，并可通过 `x-default-selected` 配置默认选中项；省略时默认全选，空数组表示默认不选。例如：
