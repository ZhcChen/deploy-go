...
Git 来源的 `source_materialization` 是独立的来源级部署契约，不属于参数
Schema，也不属于部署后验证配置。它用于声明 Agent 物化构建输入的范围；未配置
时使用 `{"mode":"full","paths":[]}`，只有显式 `sparse` 才启用 allowlist。

JSON 编辑入口在「应用详情 → 运行配置 → 部署契约」（管理员点「编辑契约」），
也可以在「从模板创建应用」向导中按模板预填后修改。JSON 不支持注释和尾逗号。
...
验证配置只允许 `http`、`tcp`、`command` 三种类型。必填字段必须完整，
`http.port` 可选，不能包含未知字段，也不能把任意 shell 文本作为命令。
...
| `port` | 可选，1 到 65535；未配置时镜像直连应用回退使用目标 `image_spec.host_port`，仍缺失则平台不做真实 HTTP 探测 |

`port` 表示节点上目标应用实际监听的本地端口，控制面平台级探测只访问
`127.0.0.1:<port>`。多模块应用（例如一次发布 worker、api 和多个页面模块的
业务应用）不应使用单个应用级 `port` 冒充所有模块的整体状态；平台级真实
探测首版适用于单目标/单入口应用，多模块应用继续以最近发布验证结果为准。
