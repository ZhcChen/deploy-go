...
- lease 状态固定为 `issued`、`granted`、`consumed`、`revoked`、`expired`：同一运行中任务在传输失败、Agent 重连或 executor 重启且 authorization 仍有效时，可在固定 TTL 内重新取得同一凭据版本；不因续传生成新密码。新 deployment retry 创建新 lease 但引用同一加密凭据版本；任务终态统一撤销未终结 lease，超出任务 deadline 不续租。
