...
trap 'printf '"'"'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.module.failed","module":"valkey","module_name":"Valkey","message":"canceled"}'"'"'\n; exit 130' TERM INT
...
service_env_file="$DEPLOY_ENV_DIR/valkey.env"
[[ -f "$service_env_file" && ! -L "$service_env_file" ]] || die deploy.preflight.failed "valkey.env is missing"
...
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.module.started","module":"valkey","module_name":"Valkey"}'

if [[ "${DEPLOY_GO_TEMPLATE_DRY_RUN:-0}" == "1" ]]; then
  printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.module.succeeded","module":"valkey","module_name":"Valkey"}'
  exit 0
fi
...
mkdir -p "$release_dir"
tar -xzf "$artifact" -C "$release_dir" compose.yaml config/valkey.conf deploy-go.yaml
install -m 0600 "$env_file" "$release_dir/compose.env"
install -m 0600 "$service_env_file" "$release_dir/valkey.env"
...
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.started","module":"valkey","step_id":"valkey.compose_config","step":"校验 Compose 配置"}'
compose config --quiet || die deploy.module.failed "compose config validation failed"
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.succeeded","module":"valkey","step_id":"valkey.compose_config","step":"校验 Compose 配置"}'

[[ -f "$DEPLOY_CANCEL_FILE" ]] && die deploy.module.failed "deployment is canceled"
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.started","module":"valkey","step_id":"valkey.up","step":"启动容器"}'
compose up -d --remove-orphans || die deploy.module.failed "docker compose up failed"
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.succeeded","module":"valkey","step_id":"valkey.up","step":"启动容器"}'

printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.started","module":"valkey","step_id":"valkey.verify","step":"等待健康检查"}'
healthy=0
...
[[ "$healthy" == "1" ]] || die deploy.module.failed "valkey did not become healthy in time"
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.succeeded","module":"valkey","step_id":"valkey.verify","step":"等待健康检查"}'
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.module.succeeded","module":"valkey","module_name":"Valkey"}'
