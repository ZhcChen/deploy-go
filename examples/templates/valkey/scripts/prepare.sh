...
mkdir -p "$output_dir"
tar -czf "$output_dir/template.tar.gz" -C "$template_root" compose.yaml config/valkey.conf deploy-go.yaml
...
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.started","module":"valkey","step_id":"valkey.package","step":"打包 Compose 配置"}'
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.succeeded","module":"valkey","step_id":"valkey.package","step":"打包 Compose 配置"}'
