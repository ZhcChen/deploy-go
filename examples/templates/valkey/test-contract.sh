...
TZ=Asia/Shanghai
EOF
cat >"$env_dir/valkey.env" <<'EOF'
VALKEY_PASSWORD=test-password
TZ=Asia/Shanghai
EOF

env \
  DEPLOY_RELEASE_VERSION=valkey-1 \
  DEPLOY_COMMIT_SHA="$commit_sha" \
  DEPLOY_MODULES=valkey \
  DEPLOY_OUTPUT_DIR="$output_dir" \
  make --no-print-directory -C "$template_root" deploy-go-prepare >"$prepare_log"

jq -e \
  --arg commit_sha "$commit_sha" \
  '.schema_version == 1 and .release_version == "valkey-1" and .commit_sha == $commit_sha and .artifacts[0].module == "valkey"' \
  "$output_dir/deploy-go-artifact.json" >/dev/null
grep -Fx 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.module.succeeded","module":"valkey","module_name":"Valkey"}' "$prepare_log" >/dev/null
...
grep -Fx 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.module.succeeded","module":"valkey","module_name":"Valkey"}' "$release_log" >/dev/null

if env \
  DEPLOY_RELEASE_VERSION=valkey-2 \
...
printf 'tampered\n' >>"$output_dir/valkey/template.tar.gz"
if env \
