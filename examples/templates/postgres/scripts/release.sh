...
trap 'printf '"'"'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.module.failed","module":"postgres","module_name":"PostgreSQL","message":"canceled"}'"'"'\n; exit 130' TERM INT
...
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.started","module":"postgres","step_id":"postgres.compose_config","step":"校验 Compose 配置"}'
compose config --quiet || die deploy.module.failed "compose config validation failed"
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.succeeded","module":"postgres","step_id":"postgres.compose_config","step":"校验 Compose 配置"}'

[[ -f "$DEPLOY_CANCEL_FILE" ]] && die deploy.module.failed "deployment is canceled"
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.started","module":"postgres","step_id":"postgres.up","step":"启动容器"}'
compose up -d --remove-orphans || die deploy.module.failed "docker compose up failed"
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.succeeded","module":"postgres","step_id":"postgres.up","step":"启动容器"}'

printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.started","module":"postgres","step_id":"postgres.verify","step":"等待健康检查"}'
healthy=0
...
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.step.succeeded","module":"postgres","step_id":"postgres.verify","step":"等待健康检查"}'
printf '%s\n' 'DEPLOY_GO_EVENT {"schema_version":1,"event":"deploy.module.succeeded","module":"postgres","module_name":"PostgreSQL"}'
