...
export function TargetDetailPage() {
  ...
  return <section className="workspace detail-page"><Link className="back-link" to={`/apps/${id}`}><ArrowLeft aria-hidden="true" />返回应用</Link><div className="detail-title"><div><h2>{target.data.environment}</h2><p><code>{target.data.scriptPath}</code></p></div><span className={`status-badge status-badge--${target.data.status === "active" ? "online" : "disabled"}`}>{target.data.status === "active" ? "启用" : "停用"}</span></div>
    <dl className="definition-grid"><div><dt>Snapshot</dt><dd><code>{target.data.snapshotHash}</code></dd></div><div><dt>节点</dt><dd><code>{target.data.nodeId}</code></dd></div></dl>
    {target.data.status === "active" ? <div className="detail-toolbar"><Link className="button button--primary" to={`/deployments/new?application=${id}&target=${targetId}`}>发起部署</Link></div> : null}
