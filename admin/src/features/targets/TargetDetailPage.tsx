...
export function TargetDetailPage() {
  ...
  return <section className="workspace detail-page"><Link className="back-link" to={`/apps/${id}`}><ArrowLeft aria-hidden="true" />返回应用</Link><div className="detail-title"><div><h2><code>{target.data.nodeId}</code></h2><p><code>{target.data.scriptPath}</code></p></div><span className={`status-badge status-badge--${target.data.status === "active" ? "online" : "disabled"}`}>{target.data.status === "active" ? "启用" : "停用"}</span></div>
