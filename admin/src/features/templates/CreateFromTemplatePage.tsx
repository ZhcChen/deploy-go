  ...
  return <section className="workspace template-wizard">
    ...
    {step === "app" ? <section className="wizard-panel">
      ...
      <form className="node-form" onSubmit={(event) => void submitApp(event)}>
        ...
        <Field label="参数 JSON Schema" hint="按模板预填；保存到应用配置，同一应用全部目标共用。modules.x-options 声明可选模块，x-default-selected 可配置默认选中模块。" className="form-span"><TextArea rows={12} spellCheck={false} value={appDraft.parameterSchema} onChange={(event) => setAppDraft({ ...appDraft, parameterSchema: event.target.value })} /></Field>
