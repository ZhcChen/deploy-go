...
export function ApplicationTemplatesPage() {
  ...
  return <section className="workspace template-page">
    ...
    <div className="template-browser">
      <aside className="template-browser__sidebar" aria-label="模板列表">
        <div className="template-browser__sidebar-heading"><h3>模板列表</h3><span>{applicationTemplates.length}</span></div>
        <div className="template-selector" role="tablist" aria-label="选择应用模板" aria-orientation="vertical">
          {applicationTemplates.map((item) => <button key={item.id} id={`template-tab-${item.id}`} type="button" role="tab" aria-controls={`template-panel-${item.id}`} aria-selected={template?.id === item.id} onClick={() => { setTemplateId(item.id); setFilePath(item.files[0].path); }}><Layers aria-hidden="true" /><span><strong>{item.name}</strong><small>{item.summary}</small></span></button>)}
        </div>
      </aside>
      {template && activeFile ? <section id={`template-panel-${template.id}`} className="template-browser__detail" role="tabpanel" aria-labelledby={`template-detail-title-${template.id}`}>
        <header className="template-detail-heading"><div><h3 id={`template-detail-title-${template.id}`}>{template.name}</h3><p>{template.summary}</p></div><span>{template.files.length} 个文件</span></header>
        ...
        <section className="template-file-viewer">
          <div className="section-heading"><div><h3><code>{activeFile.path}</code></h3><p>文件为只读示例；compose.env 与服务级 Env 的正式值通过应用配置加密登记。</p></div></div>
