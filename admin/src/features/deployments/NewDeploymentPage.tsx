...
export function NewDeploymentPage() {
  ...
  return <section className="workspace deployment-create">
    ...
    {preview.data ? <section className="deployment-preview" aria-label="部署预览">
      ...
      <dl className="definition-grid"><div><dt>应用</dt><dd>{preview.data.applicationName}</dd></div><div><dt>目标数量</dt><dd>{preview.data.targets.length}</dd></div><div><dt>执行模式</dt><dd>{preview.data.executionMode === "two_stage" ? "两阶段（prepare + release）" : "单脚本"}</dd></div>{preview.data.executionMode === "two_stage" ? <><div><dt>固定分支</dt><dd><code>{preview.data.deploymentBranch}</code></dd></div><div><dt>Commit</dt><dd><code>{preview.data.resolvedCommitSha}</code></dd></div><div><dt>发布版本</dt><dd><code>{preview.data.releaseVersion}</code></dd></div><div><dt>模块</dt><dd>{preview.data.modules?.join(", ")}</dd></div></> : null}<div><dt>Snapshot</dt><dd><code>{preview.data.snapshotHash}</code></dd></div><div><dt>参数</dt><dd><code>{JSON.stringify(preview.data.parameters)}</code></dd></div></dl>
      <ul className="deployment-target-preview" aria-label="目标节点预览">{preview.data.targets.map((target) => <li key={target.targetId}><div className="target-preview__identity"><Server aria-hidden="true" /><span><strong>{target.nodeName}</strong><code>{target.nodeId}</code></span></div><div className="target-preview__states"><span className={`status-badge status-badge--${target.agentOnline ? "online" : "pending"}`}>{target.agentOnline ? "在线" : "离线，部署将等待节点恢复"}</span><span className={`status-badge status-badge--${target.envGateStatus === "failed" ? "disabled" : target.envGateStatus === "ready" || target.envGateStatus === "not_required" ? "online" : "pending"}`}>{envGateLabel(target.envGateStatus)}</span></div><code>{target.scriptPath}</code></li>)}</ul>
