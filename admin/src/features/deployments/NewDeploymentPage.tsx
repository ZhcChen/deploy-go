...
export function NewDeploymentPage() {
  ...
  return <section className="workspace deployment-create"><Link className="back-link" to="/deployments"><ArrowLeft aria-hidden="true" />返回部署</Link><div className="workspace-heading"><div><h2>发起部署</h2><p>先生成服务端预览，再核对 snapshot 并确认执行。</p></div></div>
    ...
    {preview.data ? <section className="deployment-preview" aria-label="部署预览"><div className="section-heading"><div><h3>3. 核对并确认</h3><p>配置变化会使当前 snapshot 失效。</p></div></div><dl className="definition-grid"><div><dt>应用</dt><dd>{preview.data.applicationName}</dd></div><div><dt>节点</dt><dd>{preview.data.nodeName}</dd></div><div><dt>环境</dt><dd>{preview.data.environment}</dd></div><div><dt>执行模式</dt><dd>{preview.data.executionMode === "two_stage" ? "两阶段（prepare + release）" : "单脚本"}</dd></div>{preview.data.executionMode === "two_stage" ? <><div><dt>固定分支</dt><dd><code>{preview.data.deploymentBranch}</code></dd></div><div><dt>Commit</dt><dd><code>{preview.data.resolvedCommitSha}</code></dd></div><div><dt>发布版本</dt><dd><code>{preview.data.releaseVersion}</code></dd></div><div><dt>模块</dt><dd>{preview.data.modules?.join(", ")}</dd></div></> : <div><dt>脚本</dt><dd><code>{preview.data.scriptPath}</code></dd></div>}<div><dt>Snapshot</dt><dd><code>{preview.data.snapshotHash}</code></dd></div><div><dt>参数</dt><dd><code>{JSON.stringify(preview.data.parameters)}</code></dd></div></dl>{confirm.error ? <ApiErrorNotice error={toNotice(confirm.error)} /> : null}<div className="form-actions"><Button tone="primary" disabled={confirm.isPending} onClick={confirmDeployment}><Play aria-hidden="true" />{confirm.isPending ? "正在确认..." : "确认并发起部署"}</Button></div></section> : null}
  </section>;
}
