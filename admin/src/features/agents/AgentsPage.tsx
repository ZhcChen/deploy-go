...
export function AgentsPage() {
  ...
  const create = useMutation({ mutationFn: async () => {
    ...
    return agentsApi.agentsCreate({ xCSRFToken: auth.csrfToken, createAgentRequest: { name: name.trim(), nodeId: nodeId || undefined } });
  }, onSuccess: async (result) => { setEnrollment(result); setName(""); setNodeId(""); setCreating(false); await queryClient.invalidateQueries({ queryKey: ["agents"] }); } });
  ...
  return <section className="workspace">
    ...
    {creating ? <form className="inline-form" onSubmit={(event) => void submit(event)}><label>接入节点<select disabled={create.isPending || nodes.isLoading} value={nodeId} onChange={(event) => { const value = event.target.value; setNodeId(value); const node = unboundNodes.find((item) => item.id === value); if (node) setName(node.name); }}><option value="">创建新节点</option>{unboundNodes.map((node) => <option key={node.id} value={node.id}>接管：{node.name}</option>)}</select></label><label>Agent 名称<input autoFocus required minLength={1} maxLength={80} disabled={create.isPending || Boolean(nodeId)} value={name} onChange={(event) => setName(event.target.value)} placeholder="例如：生产节点 01" /></label>{create.error ? <ApiErrorNotice error={toNotice(create.error)} /> : null}<div className="form-actions"><Button type="button" disabled={create.isPending} onClick={() => { setCreating(false); setName(""); setNodeId(""); }}>取消</Button><Button tone="primary" disabled={create.isPending}>{create.isPending ? "正在创建..." : nodeId ? "接管并生成命令" : "创建并生成命令"}</Button></div></form> : null}
