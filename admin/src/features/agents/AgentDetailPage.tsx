...
export function AgentDetailPage() {
  ...
  return <article className="detail-page"><Link className="back-link" to="/agents"><ArrowLeft aria-hidden="true" />返回 Agent</Link><div className="detail-title"><div><h2>{agent.name}</h2><p>{agent.id}</p></div><span className={`status-badge status-badge--${agent.status}`}>{agent.status === "online" ? "在线" : "离线"}</span></div>
    <section className="detail-section"><h3>运行信息</h3><dl className="definition-grid"><div><dt>环境</dt><dd>{environmentLabel(agent.environment)}</dd></div><div><dt>关联节点</dt><dd><Link className="text-link" to={`/nodes/${agent.nodeId}`}>{agent.nodeId}</Link></dd></div><div><dt>主机名</dt><dd>{agent.hostname || "从未连接"}</dd></div><div><dt>版本</dt><dd><code>{agent.agentVersion || "-"}</code></dd></div><div><dt>架构</dt><dd>{agent.architecture || "-"}</dd></div><div><dt>最后在线</dt><dd>{agent.lastSeenAt ? new Date(agent.lastSeenAt).toLocaleString("zh-CN") : "从未连接"}</dd></div><div><dt>身份状态</dt><dd>{agent.revokedAt ? "已撤销" : "有效"}</dd></div></dl></section>
    {command ? <section className="agent-command"><div className="section-heading"><div><h3>新的安装命令</h3><p>旧的未使用 enrollment token 已失效；此命令已包含一次性 token，请在 {new Date(command.enrollmentExpiresAt).toLocaleString("zh-CN")} 前在目标 Linux 节点执行。</p></div><Button onClick={() => setCommand(null)}>关闭</Button></div><ClipboardFallback value={command.installCommand} label="复制命令" /></section> : null}
