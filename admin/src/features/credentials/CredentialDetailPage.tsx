...
import { listAllNodes, nodesApi, sshCredentialsApi } from "./api";
...
export function CredentialDetailPage() {
  ...
  const nodes = useCursorCollection(["nodes", "credential-bindings"], (after) => nodesApi.nodesList({ limit: 50, after: after ?? undefined }));
  ...
  const remove = useMutation({ mutationFn: async () => {
    ...
    const latestNodes = await listAllNodes();
    if (latestNodes.some((node) => node.sshCredentialId === id)) throw new Error("该密钥仍绑定节点，请先解绑后再删除");
  ...
  const bindings = nodes.items.filter((node) => node.sshCredentialId === id);
