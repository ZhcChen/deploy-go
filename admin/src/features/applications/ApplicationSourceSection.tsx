...
export function ApplicationSourceSection({ applicationId, isAdministrator, applicationActive }: { applicationId: string; isAdministrator: boolean; applicationActive: boolean }) {
  ...
  const editForm = editing && form ? <form className="source-form" onSubmit={(event) => void submitSave(event)}>
    ...
    <Field label="构建 Agent"><Select required value={form.buildAgentId} onChange={(event) => updateDraft({ buildAgentId: event.target.value })}><option value="">选择在线 Agent</option>{usableAgents.map((agent) => <option key={agent.id} value={agent.id}>{agent.name} · v{agent.agentVersion || "-"}</option>)}</Select></Field>
  ...
  return <section className="detail-section">
    <div className="section-heading"><div><h3>Git 来源</h3><p>绑定仓库、构建 Agent 与固定部署分支；普通用户只能查看已固定配置。</p></div>{isAdministrator && applicationActive && !editing ? <Button onClick={() => { setEditing(true); setDraft(null); }}><GitBranch aria-hidden="true" />配置来源</Button> : null}</div>
