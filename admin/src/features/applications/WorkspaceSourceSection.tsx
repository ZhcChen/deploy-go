...
export function WorkspaceSourceSection({ applicationId, isAdministrator, applicationActive }: { applicationId: string; isAdministrator: boolean; applicationActive: boolean }) {
  ...
  return <section className="detail-section">
    <div className="section-heading"><div><h3>本地工作区来源</h3><p>脚本两阶段（two_stage_script）模式不依赖 Git，prepare 在构建 Agent 的固定工作区执行。</p></div>{isAdministrator && applicationActive && !editing ? <Button onClick={() => { setEditing(true); setDraft(null); }}><FolderGit2 aria-hidden="true" />配置工作区来源</Button> : null}</div>
