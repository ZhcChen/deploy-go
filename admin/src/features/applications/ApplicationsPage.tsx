...
export function ApplicationsPage() {
  ...
  return <section className="workspace applications-page">
    ...
    <div className="filter-bar applications-filter-bar"><label><span>应用状态</span><Select value={status} onChange={(event) => { setStatus(event.target.value); setPageIndex(0); }}><option value="">全部</option><option value="active">启用</option><option value="archived">已归档</option></Select></label><label><span>环境</span><Select value={environmentFilter} onChange={(event) => { setEnvironmentFilter(event.target.value); setPageIndex(0); }}><option value="">全部环境</option>{AGENT_ENVIRONMENTS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></label>{availableTags.length > 0 ? <div className="tag-filter-block"><span>标签</span><div className="tag-filter" role="group" aria-label="按标签筛选"><button type="button" className={`tag-filter-option${tagFilter === "" ? " is-selected" : ""}`} onClick={() => { setTagFilter(""); setPageIndex(0); }}>全部</button>{availableTags.map((tag) => <button type="button" key={tag} className={`tag-filter-option${tagFilter === tag ? " is-selected" : ""}`} onClick={() => { setTagFilter(tag === tagFilter ? "" : tag); setPageIndex(0); }}>{tag}</button>)}</div></div> : null}</div>
