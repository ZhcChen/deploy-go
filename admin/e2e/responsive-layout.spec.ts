...
async function authenticate(page: Page) {
  ...
  await page.route("**/api/v1/nodes/node-1/telemetry", (route) => json(route, {
    ...
    history:[0,1,2,3].map((index)=>({received_at:new Date(Date.UTC(2026,7,16,0,0,index*30)).toISOString(),cpu_usage_ratio:.2+index*.04,memory_used_bytes:6000000000+index*100000000,work_root_used_bytes:210000000000,disk_read_bytes_per_second:800000+index*100000,disk_write_bytes_per_second:400000,disk_busy_ratio:.05,network_receive_bytes_per_second:1800000+index*100000,network_transmit_bytes_per_second:900000}))
  }));
