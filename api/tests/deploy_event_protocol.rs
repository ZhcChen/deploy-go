...
async fn deployment_events_are_authorized_paginated_and_whitelisted() {
    ...
    sqlx::query("INSERT INTO deployment_targets(id,application_id,node_id,environment,script_path,timeout_seconds,status) VALUES('target_events','app_events','node_events','test','/srv/apps/deploy.sh',60,'active')").execute(&pool).await.unwrap();
...
async fn progress_events_are_idempotent_and_bound_to_their_deployment() {
    ...
    sqlx::query("INSERT INTO agents(id,node_id,registered_at,last_seen_at,agent_version,protocol_version,capabilities_json,connection_generation) VALUES('agent_progress','node_progress','2026-08-03T00:00:00Z','2026-08-03T00:00:00Z','0.2.0',11,'[\"pty_terminal\",\"privileged_release\"]',2)").execute(&pool).await.unwrap();
    sqlx::query("INSERT INTO deployment_targets(id,application_id,node_id,environment,script_path,timeout_seconds,status) VALUES('target_progress','app_progress','node_progress','test','/srv/apps/deploy.sh',60,'active')").execute(&pool).await.unwrap();
