...
async fn image_seed(pool: &SqlitePool) {
    ...
    sqlx::query("INSERT INTO nodes(id,name,work_root,secrets_root,status) VALUES('node_image','Image Node','/srv/apps','/srv/secrets','online')")
        ...
        .await
        .unwrap();
    sqlx::query("INSERT INTO agents(id,node_id,registered_at,last_seen_at,agent_version,protocol_version,capabilities_json) VALUES('agent_image','node_image','2026-08-11T00:00:00Z','2026-08-11T00:00:00Z','0.3.0',11,'[\"pty_terminal\",\"privileged_release\"]')")
        .execute(pool)
        .await
        .unwrap();
...
async fn seed(pool: &SqlitePool) {
    ...
    sqlx::query("INSERT INTO agents(id,node_id,registered_at,last_seen_at,agent_version,protocol_version,capabilities_json,connection_generation) VALUES('agent_two','node_two','2026-08-03T00:00:00Z','2026-08-03T00:00:00Z','0.2.0',11,'[\"pty_terminal\",\"privileged_release\"]',2)").execute(pool).await.unwrap();
...
async fn preview_fails_when_build_agent_offline() {
    ...
    .unwrap();
    sqlx::query(
        "INSERT INTO agents(id,node_id,registered_at,last_seen_at,agent_version,protocol_version,capabilities_json,connection_generation) VALUES('agent_build','node_build','2026-08-12T00:00:00Z','2026-08-12T00:00:00Z','0.2.0',11,'[\"pty_terminal\",\"privileged_release\"]',1)",
    )
    .execute(&pool)
    .await
    .unwrap();
