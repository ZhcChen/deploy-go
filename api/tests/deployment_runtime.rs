...
async fn fixture(node_status: &str) -> (AppState, sqlx::SqlitePool) {
    ...
    sqlx::query("INSERT INTO agents(id,node_id,registered_at,last_seen_at,agent_version,protocol_version,capabilities_json) VALUES('agent_runtime','node_runtime','2026-08-03T00:00:00Z','2026-08-03T00:00:00Z','0.1.0',11,'[\"pty_terminal\",\"privileged_release\"]')").execute(&pool).await.unwrap();
