...
async fn serves_versioned_deployer_release_artifacts_from_api() {
    ...
    for version in ["0_3_2", "0.3.2"] {
        let manifest = get_json(
            app.clone(),
            &format!("/api/v1/deployer/download/{version}/manifest.json"),
        )
        .await;
        assert_eq!(manifest["deployer_version"], "0.3.2");
        for artifact in manifest["artifacts"].as_array().unwrap() {
            let arch = artifact["architecture"].as_str().unwrap();
            assert_eq!(
                artifact["url"],
                format!(
                    "https://deploy.example.test/api/v1/deployer/download/0_3_2/deployer/{arch}"
                )
            );
        }
    }
    ...
    for (uri, expected) in [
        (
            "/api/v1/deployer/download/0_3_2/deployer/x86_64",
            "fixture-x86_64-deployer\n",
        ),
        (
            "/api/v1/deployer/download/0.3.2/deployer/aarch64",
...
async fn rejects_unknown_deployer_version_or_architecture() {
    ...
    for uri in [
        ...
        "/api/v1/deployer/download/0_3_2/deployer/riscv64",
        "/api/v1/deployer/download/0_3_2/deployer/../manifest.json",
    ] {
        let response = app
            .clone()
            .oneshot(Request::builder().uri(uri).body(Body::empty()).unwrap())
